/** 5×7 bitmap glyphs. Bit 4 is the leftmost pixel of each row. */
...
export function pixelTextHeight(scale: number): number {
  return 7 * scale;
}
...
export function drawPixelText(
  ...
  const width = pixelTextWidth(upper, scale);
  let cx = Math.round(x);
  ...
  for (const ch of upper) {
    ...
    for (let row = 0; row < 7; row++) {
      ...
      for (let col = 0; col < 5; col++) {
        if (bits & (1 << (4 - col))) {
          ctx.fillRect(cx + col * scale, gy + row * scale, scale, scale);
        }
      }
    }
    cx += 6 * scale;
